...
#
  def read_next(f, bytes, endian)
    ...
    # # Could also maybe due something where unpack to separate binary int representations of the hex characters
    # arr = f.read(bytes).unpack('C*')
    # # => [2, 0]
    # # Then would maybe make more clear about how you swap each of the binary things around
    # if endian == :little
    #   i = arr.length - 1
    #   # ">" b/c if on 0 then nothing to swap with, so leave as is?
    #   while i > 0 do
    #     arr[i], arr[i-1] = arr[i-1], arr[i]
    #     i -= 2
    #   end
    #
    # # Then would need to do something where you convert them to the hexedecimal character representation
    # # into the hexedecimal string though
    # # NOTE STILL NEED TO FIX THAT DOES NOT TACK ON oS so need to format output
    # arr.map{ |i| i.to_s(16) }.join("")
...
class PcapReader
  ...
  def initialize(file_name = "/Users/prang/Downloads/01-net.cap")
